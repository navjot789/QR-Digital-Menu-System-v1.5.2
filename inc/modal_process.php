...
$stmt2 = $conn->prepare("SELECT ctime FROM reset_counter");  //Getting time from resetcount for prevent login when admin reset the tables..
$stmt2->execute();
$stmt2->store_result();  
$stmt2->bind_result($ctime);
$stmt2->fetch();
$db_time = array('reset_db_time' => $ctime);



$created = $db_time['reset_db_time']; // time that you want the link to expire 
$expire_date = date('d-m-Y H:i',strtotime('+5 minutes',strtotime($created))); //added +1 minute into the db time
//+1 day = adds 1 day
//+1 hour = adds 1 hour
